...
def build_dataloaders(device, train_seq, valid_seq, train_data, val_data, batch_size, ct_profiles):
    """
...
def train_one_epoch(model, train_loader):
    ...
    correlation = stats.pearsonr(np.concatenate(targets), np.concatenate(predictions))[0]
...
def main_train(train_seq, valid_seq, train_data, val_data, profiles, modelfile, device, config, use_wandb):
    """
    Main training loop.
    Args:
        train_seq, valid_seq, train_data, val_data: Data sequences and labels.
    ...
    train_loader, valid_loader = build_dataloaders(device, train_seq, valid_seq, train_data, val_data, config['batch_size'], profiles)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--train_seq", type=str, required=True, help="Path to training sequences file")
    parser.add_argument("--valid_data", type=str, required=True, help="Path to validation data file")
    parser.add_argument("--valid_seq", type=str, required=True, help="Path to validation sequences file")
    parser.add_argument("--profiles", type=str, required=True, help="Path to cell type profiles file")
    parser.add_argument("--modelfile", type=str, required=True, help="Path to save the trained model")
    parser.add_argument("--batch_size", type=int, default=64, help="Batch size for training and validation")
    parser.add_argument("--epochs", type=int, default=200, help="Number of epochs for training")
    parser.add_argument("--project_name", type=str, default="APA-Net_Training", help="Wandb project name")
    parser.add_argument("--device", type=str, default="cuda:0", help="Device to run the training on")
    parser.add_argument("--use_wandb", type=str, default="True", help="Set to True to use wandb logging, False to disable it")
    ...
    "Matt_heads": 8,
    "Matt_drop": 0.2,
    "fc1_dims": [8192, 4048, 1024, 512, 256],  # first dimension will be calculated dynamically
